...
contract DexPair is IDexPair, DexKIP7 {
    ...
    function initialize(address _token0, address _token1) external {
        ...
        name = string(
            abi.encodePacked(
                IKIP7Metadata(_token0).symbol(),
                "-",
                IKIP7Metadata(_token1).symbol(),
                " JDEX LP"
            )
        );
    }
    ...
    function mint(address to) external lock returns (uint256 liquidity) {
        ...
        if (liquidity == 0)
            revert InsufficientLiquidity("DEX: INSUFFICIENT_LIQUIDITY_MINTED");
        _mint(to, liquidity);
        ...
        returns (uint256 amount0, uint256 amount1)
    {
        ...
        if (amount0 == 0 || amount1 == 0)
            revert InsufficientLiquidity("DEX: INSUFFICIENT_LIQUIDITY_BURNED");
    ...
    function swap(
        ...
        if (amount0Out == 0 && amount1Out == 0)
            revert InsufficientAmount("DEX: INSUFFICIENT_OUTPUT_AMOUNT");
        (uint112 _reserve0, uint112 _reserve1, ) = getReserves(); // gas savings
        if (amount0Out >= _reserve0 || amount1Out >= _reserve1)
            revert InsufficientLiquidity("DEX: INSUFFICIENT_LIQUIDITY");
        ...
        uint256 balance1;
        {
            ...
            if (to == _token0 || to == _token1)
                revert InvalidAddressParameters("DEX: INVALID_TO");
            if (amount0Out > 0) _safeTransfer(_token0, to, amount0Out); // optimistically transfer tokens
            if (amount1Out > 0) _safeTransfer(_token1, to, amount1Out); // optimistically transfer tokens
            if (data.length > 0)
                IDexCallee(to).uniswapV2Call(
                    msg.sender,
                    amount0Out,
                    amount1Out,
                    data
        ...
        if (amount0In == 0 && amount1In == 0)
            revert InsufficientAmount("DEX: INSUFFICIENT_INPUT_AMOUNT");
        {
    ...
    function skim(address to) external lock {
        ...
        _safeTransfer(
            _token0,
            to,
            IKIP7(_token0).balanceOf(address(this)) - reserve0
        );
        _safeTransfer(
            _token1,
            to,
            IKIP7(_token1).balanceOf(address(this)) - reserve1
        );
    }
